...
//            }
    ...
    open fun subscribe(applicationId: String,playerID:String, completion:((response:JsonElement?) -> Unit?)?, failure:((code:Int, message:String?) -> Unit?)?) : PDYRequestSingleton {
        if (this.context != null) {
            val request = PDYRequestSingleton.getInstance(context!!)
            request.post("${this.baseUrl()}/application/${applicationId}/player/${playerID}/subscribe", this.defaultHeaders(), null, { response:JsonElement? ->
                completion?.invoke(response)
            }, { code:Int, message:String? ->
                failure?.invoke(code, message)
            })
    ...
    fun trackBanner(applicationId: String, playerID: String, data: JsonElement?, completion:((response:JsonElement?) -> Unit?)?, failure:((code:Int, message:String?) -> Unit?)?): PDYRequestSingleton {
        if (this.context != null) {
            val request = PDYRequestSingleton.getInstance(context!!)
            request.post("${this.baseUrl()}/application/${applicationId}/player/${playerID}/subscribe", this.defaultHeaders(), data, { response:JsonElement? ->
                completion?.invoke(response)
            }, { code:Int, message:String? ->
                failure?.invoke(code, message)
            })
